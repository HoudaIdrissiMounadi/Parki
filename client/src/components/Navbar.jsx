    ...
    <nav className={`sticky top-0 z-[1001] w-full transition-all duration-300 ${
      ...
      <div className="max-w-7xl mx-auto px-4 flex justify-between items-center">
        <Link to="/" className="flex items-center gap-2 group">
          <div className="bg-gray-900 p-2 rounded-xl group-hover:rotate-12 transition-transform">
        ...
        <div className="hidden md:flex items-center gap-8">
          {currentLinks.map((link, i) => (
            <Link key={i} to={link.path} className="font-bold text-gray-600 hover:text-gray-900 transition-colors text-sm">
              {link.name}
            </Link>
          ))}
          {isAuthenticated ? (
            ...
            <div className="flex items-center gap-4">
              ...
              <Link to="/register" className="bg-gray-900 text-white font-black px-6 py-3 rounded-xl hover:bg-black transition-all shadow-lg active:scale-95">
                S'inscrire
              </Link>
            </div>
          )}
        ...
        <div className="md:hidden absolute top-full left-0 w-full bg-white border-t border-gray-100 p-4 shadow-xl space-y-2 animate-in slide-in-from-top duration-300">
          {currentLinks.map((link, i) => (
            <Link key={i} to={link.path} className="flex items-center gap-3 p-4 font-bold text-gray-900 bg-gray-50 rounded-2xl" onClick={() => setIsOpen(false)}>
              <link.icon size={20} className="text-gray-900" /> {link.name}
            </Link>
          ))}
          {isAuthenticated ? (
            ...
            <div className="grid grid-cols-2 gap-4 pt-2">
              ...
              <Link to="/register" className="p-4 text-center font-bold text-white bg-gray-900 rounded-2xl" onClick={() => setIsOpen(false)}>S'inscrire</Link>
            </div>
          )}
        </div>
      )}
