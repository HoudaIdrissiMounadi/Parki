...
const Button = ({ children, variant = 'primary', size = 'md', className, isLoading, ...props }) => {
  const variants = {
    primary: 'bg-primary text-white hover:bg-blue-700 shadow-lg shadow-blue-100',
    secondary: 'bg-white text-gray-900 border border-gray-200 hover:bg-gray-50',
    accent: 'bg-accent text-white hover:bg-red-600 shadow-lg shadow-red-100',
    ghost: 'bg-transparent text-gray-600 hover:bg-gray-100',
  };
  const sizes = { sm: 'px-4 py-2 text-sm rounded-lg', md: 'px-6 py-3 rounded-xl', lg: 'px-8 py-4 text-lg rounded-2xl' };
  return (
    <button className={cn('inline-flex items-center justify-center font-bold transition-all active:scale-95 disabled:opacity-50 disabled:pointer-events-none', variants[variant], sizes[size], className)} disabled={isLoading} {...props}>
