...
const Home = () => {
  ...
  // Default to local time in YYYY-MM-DDTHH:MM format
  const now = new Date();
  ...
  return (
    <div className="space-y-24 pb-24 overflow-x-hidden">
      ...
      <section className="relative h-[85vh] min-h-[600px] flex items-center justify-center px-4 overflow-hidden bg-gray-900">
        ...
        <div className="relative z-10 w-full max-w-5xl text-center px-4">
          ...
          <motion.form
            ...
            className="bg-white p-3 rounded-[32px] md:rounded-full shadow-2xl flex flex-col md:flex-row items-center gap-2 max-w-4xl mx-auto"
          >
            ...
            <div className="flex-1 flex items-center gap-3 px-6 w-full">
              ...
              <input
                type="datetime-local"
                className="w-full py-4 text-lg font-bold text-gray-400 focus:text-gray-900 outline-none bg-transparent cursor-pointer"
                value={dateTime}
          ...
          >
            <div className="flex items-center gap-2 text-white/90 font-bold">
              <Shield className="text-primary" size={20} /> Paiement Sécurisé
            </div>
            <div className="flex items-center gap-2 text-white/90 font-bold">
              <Star className="text-primary fill-primary" size={20} /> Parkings Vérifiés
            </div>
            <div className="flex items-center gap-2 text-white/90 font-bold">
              <Zap className="text-primary" size={20} /> Accès Instantané
            </div>
          </motion.div>
        </div>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-4">
        ...
        <motion.div variants={containerVariants} initial="hidden" whileInView="visible" viewport={{ once: true }} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
          {popularCities.map((city) => (
            <motion.div key={city.name} variants={itemVariants} whileHover={{ y: -10 }} className="group relative h-80 rounded-[32px] overflow-hidden cursor-pointer shadow-xl" onClick={() => navigate(`/search?city=${city.name}`)}>
      ...
      <section className="bg-white py-24">
        <div className="max-w-7xl mx-auto px-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-16">
            {[
              { step: '01', title: 'Recherchez', desc: 'Trouvez le parking idéal à proximité de votre destination.', icon: Search, bg: 'bg-blue-50 text-blue-600' },
              { step: '02', title: 'Réservez', desc: 'Choisissez votre créneau et payez en toute sécurité.', icon: Clock, bg: 'bg-purple-50 text-purple-600' },
              { step: '03', title: 'Garez-vous', desc: 'Accédez au parking avec votre QR code et profitez.', icon: Zap, bg: 'bg-orange-50 text-orange-600' },
            ].map((item, index) => (
              <motion.div key={item.step} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.2 }} className="relative text-center group">
      ...
      <section className="max-w-7xl mx-auto px-4 pb-12">
        <motion.div initial={{ opacity: 0, scale: 0.95 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} className="bg-gray-900 rounded-[48px] overflow-hidden relative p-12 md:p-24 flex flex-col md:flex-row items-center gap-12">
          <div className="relative z-10 flex-1">
            <h2 className="text-4xl md:text-5xl font-black text-white mb-6 leading-tight">Gagnez de l'argent avec <br /> votre place de parking</h2>
            <p className="text-xl text-gray-400 mb-10 font-medium">Rejoignez des milliers de propriétaires au Maroc et commencez à générer des revenus passifs dès aujourd'hui.</p>
            <div className="flex flex-col sm:flex-row gap-4">
              <Button size="lg" className="px-10">Devenir Propriétaire</Button>
              <Button variant="ghost" size="lg" className="text-white hover:bg-white/10"><Play className="mr-2" size={20} /> En savoir plus</Button>
            </div>
          </div>
          <div className="flex-1 relative">
            <div className="bg-primary/20 absolute inset-0 blur-3xl rounded-full"></div>
            <motion.img whileHover={{ rotate: 0, scale: 1.05 }} src="https://images.unsplash.com/photo-1590674867551-11c3a2df5bb8?auto=format&fit=crop&q=80" className="relative z-10 rounded-3xl shadow-2xl rotate-3 transition-all duration-500" alt="Owner" />
          </div>
        </motion.div>
      </section>
    </div>
